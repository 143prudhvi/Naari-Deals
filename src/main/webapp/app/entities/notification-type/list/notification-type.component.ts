...
import { EntityArrayResponseType, NotificationTypeService } from '../service/notification-type.service';
...
export class NotificationTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: INotificationType): number => this.notificationTypeService.getNotificationTypeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.notificationTypes = this.refineData(dataFromBody);
  }

  protected refineData(data: INotificationType[]): INotificationType[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: INotificationType[] | null): INotificationType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.notificationTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
