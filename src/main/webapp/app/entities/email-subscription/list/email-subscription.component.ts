...
import { EntityArrayResponseType, EmailSubscriptionService } from '../service/email-subscription.service';
...
export class EmailSubscriptionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEmailSubscription): number => this.emailSubscriptionService.getEmailSubscriptionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.emailSubscriptions = this.refineData(dataFromBody);
  }

  protected refineData(data: IEmailSubscription[]): IEmailSubscription[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IEmailSubscription[] | null): IEmailSubscription[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.emailSubscriptionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
