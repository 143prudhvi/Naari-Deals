...
import { EntityArrayResponseType, BioProfileService } from '../service/bio-profile.service';
...
export class BioProfileComponent implements OnInit {
  ...
  trackId = (_index: number, item: IBioProfile): number => this.bioProfileService.getBioProfileIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.bioProfiles = this.refineData(dataFromBody);
  }

  protected refineData(data: IBioProfile[]): IBioProfile[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IBioProfile[] | null): IBioProfile[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.bioProfileService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
